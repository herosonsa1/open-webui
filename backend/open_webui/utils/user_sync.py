...
from open_webui.models.users import Users, User, UserModel
...
async def sync_users_from_prod_db(db=None) -> tuple[int, int]:
    """사용자 데이터 동기화 비동기 함수.
    db: AsyncSession 객체 (전달되지 않으면 신규 획득)
    반환값: (appended_count, updated_count)
    """
    log.info("[SYNC] 사용자 데이터(TB_IT_USER -> user) 동기화 시작...")
    ...
    async with get_async_db_context(db) as session:
        for row in rows:
            ...
            # 기존 사용자 조회 (세션 내부에서 직접 로드하여 추적 가능하게 함)
            ...
            if existing_user:
                ...
                if changed:
                    ...
                    # SQLAlchemy가 자동으로 dirty tracking을 하므로 session.commit()으로 업데이트 반영됨
                    updated_count += 1
            else:
                # 신규 사용자 가입 등록 처리
                ...
                new_auth = Auth(
                    ...
                    password=hashed_pw,
                    active=True,
                    password_updated_at=0 # 최초 로그인 시 비밀번호 변경 유도
                )
...
async def user_sync_scheduler_loop(app) -> None:
    ...
    while True:
        try:
            ...
            # 동기화 시작
            appended, updated = await sync_users_from_prod_db()
